...
import info.appdev.charting.jobs.AnimatedMoveViewJob.Companion.getInstance
import info.appdev.charting.jobs.AnimatedZoomJob.Companion.getInstance
import info.appdev.charting.jobs.MoveViewJob.Companion.getInstance
import info.appdev.charting.jobs.ZoomJob.Companion.getInstance
import info.appdev.charting.listener.BarLineChartTouchListener
...
abstract class BarLineChartBase<T : BarLineScatterCandleBubbleData<*, *>> : Chart<T>,
    ...
    protected var axisRendererLeft: YAxisRenderer
    protected var axisRendererRight: YAxisRenderer
    ...
    protected var xAxisRenderer: XAxisRenderer
    ...
    init {
        axisRendererLeft = YAxisRenderer(viewPortHandler, mAxisLeft, mLeftAxisTransformer)
        axisRendererRight = YAxisRenderer(viewPortHandler, mAxisRight, mRightAxisTransformer)
    ...
    public override fun calculateOffsets() {
        if (!mCustomViewPortEnabled) {
            ...
            if (mAxisLeft.needsOffset()) {
                offsetLeft += mAxisLeft.getRequiredWidthSpace(axisRendererLeft.paintAxisLabels)
            }

            if (mAxisRight.needsOffset()) {
                offsetRight += mAxisRight.getRequiredWidthSpace(axisRendererRight.paintAxisLabels)
            }
     ...
     */
    fun getEntryByTouchPoint(x: Float, y: Float): BaseEntry<Float>? {
    ...
    /**
     * Custom axis renderer for the right axis and overwrites the existing one.
     */
    ...
    fun setTextColor(color: Int) {
        axisRendererLeft.setTextColor(color)
        axisRendererRight.setTextColor(color)
        xAxisRenderer.setTextColor(color)
    }
}
