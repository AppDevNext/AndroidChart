 ...
 */
class PieChart2 : PieRadarChartBase<PieData> {
    ...
    private fun calcAngles() {
        ...
        val yValueSum: Float = mData!!.yValueSum
        ...
        for (i in 0..<mData.dataSetCount) {
            val set: IPieDataSet = dataSets.get(i)
    ...
    fun needsHighlight(index: Int): Boolean {
        ...
        for (highlight in mIndicesToHighlight) if (highlight.getX() as Int == index) return true
    ...
    @get:Deprecated("PieChart has no XAxis")
    ...
    val accessibilityDescription: String
        get() {
            ...
            for (i in 0..<entryCount) {
                val entry: PieEntry = pieData.getDataSet().getEntryForIndex(i)
                val percentage: Float = (entry.getValue() / pieData.getYValueSum()) * 100
                builder.append(
                    java.lang.String.format(
                        Locale.getDefault(), "%s has %.2f percent pie taken",
                        (if (TextUtils.isEmpty(entry.getLabel())) "No Label" else entry.getLabel()),
                        percentage
                    )
                )
            }
