...
open class CombinedChartRenderer(chart: CombinedChart, animator: ChartAnimator, viewPortHandler: ViewPortHandler) : DataRenderer(animator, viewPortHandler) {
    ...
    fun createRenderers() {
        ...
        orders.forEach { order ->
            when (order) {
                DrawOrder.BAR -> if (chart.barData != null) mRenderers.add(BarChartRenderer(chart, mAnimator, viewPortHandler))
                DrawOrder.BUBBLE -> if (chart.bubbleData != null) mRenderers.add(BubbleChartRenderer(chart, mAnimator, viewPortHandler))
                DrawOrder.LINE -> mRenderers.add(LineChartRenderer(chart, mAnimator, viewPortHandler))
                DrawOrder.CANDLE -> if (chart.candleData != null) mRenderers.add(CandleStickChartRenderer(chart, mAnimator, viewPortHandler))
                DrawOrder.SCATTER -> if (chart.scatterData != null) mRenderers.add(ScatterChartRenderer(chart, mAnimator, viewPortHandler))
            }
        }
    }
    ...
    override fun drawHighlighted(c: Canvas, indices: Array<Highlight>) {
        ...
        for (renderer in mRenderers) {
            ...
            else if (renderer is CandleStickChartRenderer) data = renderer.mChart.candleData
            else if (renderer is ScatterChartRenderer) data = renderer.mChart.scatterData
            else if (renderer is BubbleChartRenderer) data = renderer.mChart.bubbleData
