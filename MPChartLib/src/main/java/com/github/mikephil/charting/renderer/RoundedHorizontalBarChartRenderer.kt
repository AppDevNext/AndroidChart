 ...
 * @noinspection unused
 */
open class RoundedHorizontalBarChartRenderer(chart: BarDataProvider, animator: ChartAnimator?, viewPortHandler: ViewPortHandler?) :
    ...
    private fun roundRect(rect: RectF, rx: Float, ry: Float, tl: Boolean, tr: Boolean, br: Boolean, bl: Boolean): Path {
        ...
        val widthMinusCorners = (width - (2 * rxLocal))
        val heightMinusCorners = (height - (2 * ryLocal))

        path.moveTo(right, top + ryLocal)
        if (tr) {
            path.rQuadTo(0f, -ryLocal, -rxLocal, -ryLocal) //top-right corner
        } else {
            path.rLineTo(0f, -ryLocal)
            path.rLineTo(-rxLocal, 0f)
        }
        path.rLineTo(-widthMinusCorners, 0f)
        if (tl) {
            path.rQuadTo(-rxLocal, 0f, -rxLocal, ryLocal) //top-left corner
        } else {
            path.rLineTo(-rxLocal, 0f)
            path.rLineTo(0f, ryLocal)
        }
        ...
        if (bl) {
            path.rQuadTo(0f, ryLocal, rxLocal, ryLocal) //bottom-left corner
        } else {
            path.rLineTo(0f, ryLocal)
            path.rLineTo(rxLocal, 0f)
        }
        ...
        path.rLineTo(widthMinusCorners, 0f)
        if (br) {
            path.rQuadTo(rxLocal, 0f, rxLocal, -ryLocal) //bottom-right corner
        } else {
            path.rLineTo(rxLocal, 0f)
            path.rLineTo(0f, -ryLocal)
        }
