...
/** @noinspection unused
 */
open class RoundedBarChartRenderer(chart: BarDataProvider, animator: ChartAnimator?, viewPortHandler: ViewPortHandler?) :
    ...
    private fun roundRect(rect: RectF, rx: Float, ry: Float, tl: Boolean, tr: Boolean, br: Boolean, bl: Boolean): Path {
        ...
        path.rLineTo(widthMinusCorners, 0f)
        if (br) path.rQuadTo(rxLocal, 0f, rxLocal, -ryLocal) //bottom-right corner
        else {
            path.rLineTo(rxLocal, 0f)
            path.rLineTo(0f, -ryLocal)
        }
